...
app.MapGet("/board", () =>
{
    ...
    using (var connection = new NpgsqlConnection(connectionString))
    {
        ...
        var command = connection.CreateCommand();
        command.CommandText = @"SELECT id, port_number, ip_address, port_status, other, power_socket FROM board";
...
app.MapPost("/board",async (HttpRequest request) => {
   ...
   var command = connection.CreateCommand(); 
   command.CommandText = @"
        INSERT INTO board (id, port_number, ip_address, port_status, other, power_socket) 
        VALUES (@id, @port_number, @ip_address, @port_status, @other, @power_socket)";  // Using named parameters
